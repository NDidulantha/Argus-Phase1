...
from argus.services.graph_builder import build_graph
...
async def ingest_events(
    ...
    for payload in payloads:
        ...
        try:
            ...
            try:
                async with session.begin_nested():
                    await build_graph(session, tenant_id, normalized_event)
            except Exception as exc:  # noqa: BLE001 - graph building is non-critical
                log.warning("graph_building_failed", error=str(exc)[:200])
